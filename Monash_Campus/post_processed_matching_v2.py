...
DEFAULT_INPUT_CSV = "Data/edited_100926.csv"
OUTPUT_DIR = "Data"
...
# Route .rou.xml to read the ground-truth edge list from -- the "edges"
# attribute of a <route> element, e.g. the one duarouter/netedit wrote for
# full_campus_v3_rou.xml. Path resolves relative to this script's own
# directory, same convention as SUMO_CFG. Set to None to skip file loading
# and use GROUND_TRUTH_ROUTE below instead.
GROUND_TRUTH_ROUTE_FILE = r"Monash_Campus\2026-08-25-19-43-30\full_campus_v3.rou.xml"

# Which <route id="..."> to use if the file defines more than one. None
# picks the first <route> element in the file, which is fine for a
# single-route file like the one above.
GROUND_TRUTH_ROUTE_ID = None

# Manual fallback: fill this in directly instead of using a file, or to
# hard-code a route regardless of what --route-file is passed. Whichever
# of file / this list is actually available is resolved in main().
GROUND_TRUTH_ROUTE = []

GT_METHOD_KEY = "generate_gt"
GT_OUTPUT_NAME = "ground_truth.csv"
GT_BACK_TOLERANCE_M = 5.0
# ------------------------------------
...
METHODS = {
    ...
    "st": {
        ...
        # NOT a map-matching method. Projects each fix onto GROUND_TRUTH_ROUTE
    # to produce the reference track the other five are scored against.
    # It lives in this registry anyway so GT is generated by the exact same
    # harness -- same CSV loader, same chronological sort, same convertGeo,
    # same output schema -- as the methods it will be compared with. A GT
    # file built by a separate script would differ from the method outputs
    # in ways that are invisible until the CMP numbers look wrong.
    GT_METHOD_KEY: {
        "module": "route_matcher",
        "cls": "RouteMatcher",
        "kwargs": {
            "route_edges": GROUND_TRUTH_ROUTE,
            "back_tolerance_m": GT_BACK_TOLERANCE_M,
            "vclass": None,
        },
        "label": "Ground truth (project onto predefined route)",
    },
}
...
METHOD_ALIASES = {
    ...
    "st-matching": "st",
    "gt": GT_METHOD_KEY,
    "groundtruth": GT_METHOD_KEY,
    "ground_truth": GT_METHOD_KEY,
    "ground-truth": GT_METHOD_KEY,
    "route": GT_METHOD_KEY,
}
...
def prompt_for_method():
    ...
    for i, k in enumerate(keys, start=1):
        print(f"  {i}. {k:<12} {METHODS[k]['label']}")
    print("=" * 62)
...
def parse_route_from_rou_xml(path, route_id=None):
    """
    Pull an ordered edge list out of a SUMO .rou.xml's <route edges="...">
    attribute, so GROUND_TRUTH_ROUTE never has to be retyped by hand.

    route_id picks which <route> to use when the file defines more than
    one; None uses the first <route> element found, which is correct for
    a file built specifically as a GT reference (a single named route plus
    a reference vehicle following it, like full_campus_v3_rou.xml).

    Edge validity against the actual network is NOT checked here --
    RouteMatcher.__init__ already does that (via net.getEdge()) once the
    matcher is built, so there's no reason to duplicate it.
    """
    ...
    if not routes:
        raise ValueError(f"No <route> element found in {path}")

    if route_id is not None:
        matches = [r for r in routes if r.get("id") == route_id]
        if not matches:
            found = ", ".join(r.get("id", "<no id>") for r in routes)
            raise ValueError(
                f"No <route id=\"{route_id}\"> found in {path}. "
                f"Routes present: {found}"
            )
        route_el = matches[0]
    else:
        if len(routes) > 1:
            found = ", ".join(r.get("id", "<no id>") for r in routes)
            print(f"[WARN] {path} defines {len(routes)} <route> elements "
                  f"({found}); using the first one "
                  f"('{routes[0].get('id', '<no id>')}'). Pass --route-id "
                  f"to pick a specific one.")
        route_el = routes[0]

    edges_raw = route_el.get("edges")
    if not edges_raw:
        raise ValueError(
            f"<route id=\"{route_el.get('id')}\"> in {path} has no "
            f"'edges' attribute."
        )

    # Whitespace-separated, and per this file, can include blank lines
    # inside the attribute value -- bare split() handles both.
    return edges_raw.split()
...
def default_output_path(method_key, use_kalman):
    ...
    Ground truth is the exception. There is only ever one GT for a given
    ride, and the CMP script needs a single predictable path to compare
    every method against, so it gets a fixed name. Regenerating it
    overwrites -- announced loudly, since a stale GT silently left in
    place would corrupt every number downstream of it."""
    out_dir = os.path.join(SCRIPT_DIR, OUTPUT_DIR)
    os.makedirs(out_dir, exist_ok=True)

    if method_key == GT_METHOD_KEY:
        path = os.path.join(out_dir, GT_OUTPUT_NAME)
        if os.path.exists(path):
            print(f"[WARN] {path} already exists and WILL BE OVERWRITTEN. "
                  f"Pass --output to write elsewhere.")
        return path
...
def parse_args():
    ...
    p = argparse.ArgumentParser(
        ...
        epilog="Methods:\n" + "\n".join(
            f"  {k:<12} {v['label']}" for k, v in METHODS.items()
        ),
    )
    ...
    p.add_argument("--route-file", default=None,
                   help="Path to a .rou.xml with the ground-truth route "
                        "(overrides GROUND_TRUTH_ROUTE_FILE)")
    p.add_argument("--route-id", default=None,
                   help="id of the <route> to use if the file has more "
                        "than one (default: first <route> found)")
    return p.parse_args()
...
def main():
    ...
    if METHOD_KEY == GT_METHOD_KEY:
        ...
        if route_file:
            ...
            print(f"[INFO] Loaded {len(edges)} edges for ground-truth route.")
        elif GROUND_TRUTH_ROUTE:
            edges = GROUND_TRUTH_ROUTE
        else:
            raise ValueError(
                "No ground-truth route available: set "
                "GROUND_TRUTH_ROUTE_FILE (or pass --route-file), or fill "
                "in GROUND_TRUTH_ROUTE directly, before running "
                "--method generate_gt."
            )

        METHODS[GT_METHOD_KEY]["kwargs"]["route_edges"] = edges
